...
If you selected to stream, provide your Twitch or other streaming service credentials/keys when prompted. Click **Apply Settings** when finished.
...
Navigate to the extracted JUXT OBS folder and select the file `JUXT Script.lua`. This may cause OBS to crash or freeze, if so just restart it. Regardless, it may also cause pop-ups, please close any that appear. Once OBS is running again, open the **Scripts** menu.

On the right-hand side, you'll see parameters that you can adjust (such as text inputs, dropdown lists, and folder selections).
